...
# This function converts a char into the relative number (string type).
def char_to_str_n(c: str) -> str:
    return str(ord(c) - ord("a") + 1)

# Convert the string word into a int number.
# (this is not the result of the first transformation k = 1)
def convert(s: str) -> int:
    return int("".join(map(char_to_str_n, s)))

# Carries out the "transform" stage.
def int_to_int(n: int) -> int:
    return sum(map(int, str(n)))

# Transform from the string into the final integer.
def transform(s: str, k) -> int:
    s = convert(s)
    while k>0:
        s = int_to_int(s)
        k-=1
...
if __name__ == "__main__":
    print(transform("leetcode", 2)) #Expected: 6
